...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The \removed{referenced} object \added{referenced by \tcode{obj}}
\removed{shall be}\added{is} aligned to \tcode{required_alignment}.

\pnum
\effects \removed{Constructs an atomic reference that references the object.}
\added{Equivalent to: \tcode{ptr = std::addressof(obj)}.}
...
\added{Equivalent to: \tcode{ptr = ref.ptr}.}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The \tcode{order} argument \removed{shall not be}\added{is neither}
\tcode{memory_order_consume}, \added{nor}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The \tcode{failure} argument \removed{shall not be}\added{is neither}
\tcode{memory_order_release} nor \tcode{memory_order_acq_rel}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} \tcode{T} \removed{shall be}\added{is} a\removed{n} \added{complete} object type\added{.}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The \tcode{order} argument
\removed{shall not be}\added{is neither} \tcode{memory_order::consume},
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} T \removed{shall be}\added{is} a\removed{n} \added{complete} object type\added{.}
\removed{, otherwise the program is ill-formed.}
\removed{[\textit{Note}: Pointer arithmetic on \tcode{void*} or function pointers is ill-formed. \textit{\,---\,end note}]}
...
\pnum
\removed{\requires}\added{\expects} The \tcode{order} argument \removed{shall not be}\added{is neither}
\tcode{memory_order::consume}, \tcode{memory_order::acquire}, nor \tcode{memory_order::acq_rel}.
