...
is thrown then a lock \removed{shall not have}\added{has not} been acquired for the current execution agent.
\end{itemdescr}
...
exception is thrown then a lock \removed{shall not have}\added{has not} been acquired for the current execution agent.
...
\pnum
\removed{\requires}\added{\expects}\ \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} \removed{shall} meet the
