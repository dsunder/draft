...
\pnum
Implementation-provided clocks that are used for these functions \removed{shall} satisfy the
...
\pnum
The mutex types \removed{shall} satisfy the \oldconcept{Lockable} requirements\iref{thread.req.lockable.req}.
...
\pnum
The timed mutex types \removed{shall} satisfy the \oldconcept{TimedLockable}
...
are \defn{shared mutex types}. Shared mutex types \removed{shall} satisfy the requirements of
mutex types\iref{thread.mutex.requirements.mutex}\removed{,} and additionally
\removed{shall} satisfy the requirements set out below. In this description,
...
\pnum
The class \tcode{shared_mutex} \removed{shall satisfy}\added{satisfies} all of the
...
\defn{shared timed mutex type}. Shared timed mutex types \removed{shall} satisfy the requirements of
...
\pnum
The class \tcode{shared_timed_mutex} \removed{shall satisfy}\added{satisfies} all of the
...
object. The supplied \tcode{Mutex} type \removed{shall satisfy}\added{satisfies}
...
\removed{shall satisfy}\added{satisfies} the \oldconcept{BasicLockable} requirements\iref{thread.req.lockable.basic}.
Otherwise, each of the mutex types
\removed{shall} satisfy the \oldconcept{Lockable} requirements\iref{thread.req.lockable.req}.
...
\tcode{Mutex} type \removed{shall satisfy}\added{satisfies} the \oldconcept{BasicLockable}
...
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{satisfies} the \oldconcept{Lockable}
...
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{satisfies} the
...
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{satisfies} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\removed{shall satisfy}\added{satisfies} the \oldconcept{Lockable}
requirements\iref{thread.req.lockable.req}.
...
\removed{shall satisfy}\added{satisfies} the \oldconcept{TimedLockable}
requirements\iref{thread.req.lockable.timed}.
...
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type 
\removed{shall satisfy}\added{satisfies} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\tcode{Mutex} type \removed{shall satisfy}\added{satisfies} the shared mutex
...
\removed{shall satisfy}\added{satisfies} the \oldconcept{Lockable} requirements. \begin{note} The
...
\pnum
\removed{\requires}\added{\expects} Each template parameter type 
\removed{shall satisfy}\added{satisfies} the \oldconcept{Lockable} requirements,
\begin{note} The
\tcode{unique_lock} class template meets these requirements when suitably instantiated.
\end{note}
...
the current thread have been destroyed. This notification \removed{shall be as if}\added{is equilavent to}:
\begin{codeblock}
lk.unlock();
cond.notify_all();
\end{codeblock}
...
\pnum
A \tcode{Lock} type \removed{shall satisfy}\added{satisfies} the \oldconcept{BasicLockable}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{Alloc} \removed{shall satisfy}\added{satisfies} the \oldconcept{Allocator}
...
\added{\pnum
\expects \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} satisfy the \oldconcept{MoveConstructible} requirements.}
