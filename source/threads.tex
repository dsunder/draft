...
meeting its specifications. Failure to allocate storage \removed{shall be}\added{is} reported as described
...
\pnum
Implementation-provided clocks that are used for these functions \removed{shall} meet the
...
timeout\iref{thread.req.timing} specified by \tcode{rel_time}. The function \removed{shall}\added{will} not return
within the timeout specified by \tcode{rel_time} unless it has obtained a lock on \tcode{m}
for the current execution agent. If an exception is thrown then a lock \removed{shall not have}\added{has not} been
...
timeout\iref{thread.req.timing} specified by \tcode{abs_time}. The function \removed{shall}\added{will} not return
before the timeout specified by \tcode{abs_time} unless it has obtained a lock on \tcode{m} for
the current execution agent. If an exception is thrown then a lock \removed{shall not have}\added{has not} been acquired
...
\pnum
\effects
\removed{Constructs a \tcode{thread}}\added{The} object \removed{that} does not represent a thread of execution.
...
is \added{not} the same type as \removed{\tcode{std::}}\tcode{thread}.
...
\pnum
\removed{\requires}\added{\expects}\ \removed{\tcode{F}} \added{\tcode{decay_t<F>}} and each \removed{$\tcode{T}_i$} \added{type} in \removed{\tcode{Args}} \added{\tcode{decay_t<Args>}} \removed{shall} meet the
...
\tcode{%
\removed{\placeholdernc{INVOKE}}\added{invoke}(\brk{}%
...
\begin{itemdescr}
\pnum
\effects If \tcode{joinable()}, calls \tcode{terminate()}. Otherwise, has no effects.
\begin{note}
...
\pnum
\added{\expects \tcode{decay_t<F>} and each type in \tcode{decay_t<Args>} meet the
\oldconcept{MoveConstructible} requirements.}
...
\begin{codeblock}
@\removed{\placeholdernc{INVOKE}}\added{invoke}@(@\placeholdernc{decay-copy}@(std::forward<F>(f)), get_stop_token(),
       @\placeholdernc{decay-copy}@(std::forward<Args>(args))...)
\end{codeblock}
if that expression is well-formed,
otherwise
\begin{codeblock}
@\removed{\placeholdernc{INVOKE}}\added{invoke}@(@\placeholdernc{decay-copy}@(std::forward<F>(f)), @\placeholdernc{decay-copy}@(std::forward<Args>(args))...)
\end{codeblock}
with the calls to
...
of \tcode{f} will be thrown in the constructing thread, not the new thread.
\end{note}
If the \removed{\tcode{\placeholdernc{INVOKE}}} \added{\tcode{invoke}} expression exits via an exception,
...
They \removed{shall} meet the requirements set out in this subclause. In this description, \tcode{m}
denotes an object of a mutex type.

\pnum
The mutex types \removed{shall} meet the \oldconcept{Lockable} requirements\iref{thread.req.lockable.req}.
...
a \tcode{resource_deadlock_would_occur} error condition \removed{may}\added{might} be observed.
\end{note}
...
The supplied \tcode{Mutex} type \removed{shall} meet\added{s} the \oldconcept{Lockable}
...
does not own the mutex. The supplied \tcode{Mutex} type \removed{shall} meet\added{s} the
...
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.
The supplied \tcode{Mutex} type \removed{shall} meet\added{s} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} \removed{shall} meet\added{s} the \oldconcept{Lockable}
requirements\iref{thread.req.lockable.req}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type \removed{shall} meet\added{s} the \oldconcept{TimedLockable}
requirements\iref{thread.req.lockable.timed}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type \removed{shall} meet\added{s} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} Each template parameter type \removed{shall} meet\added{s} the \oldconcept{Lockable} requirements.
\begin{note}
The
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{Alloc} \removed{shall} meet\added{s} the \oldconcept{Allocator}
...
\added{\pnum
\expects \tcode{decay_t<F>} and each type in \tcode{decay_t<Args>} meet the \oldconcept{MoveConstructible} requirements.}
...
\tcode{\removed{\placeholdernc{INVOKE}}\added{invoke}(\placeholdernc{decay-copy}(std::forward<F>(f)),}
...
\tcode{\removed{\placeholdernc{INVOKE}}\added{invoke}(std::move(g), std::move(xyz))} where \tcode{g} is the stored value of
...
Once evaluation of \tcode{\removed{\placeholdernc{INVOKE}}\added{invoke}(std::move(g), std::move(xyz))} begins, the function is no longer
considered deferred.
\begin{note}
