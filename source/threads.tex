...
the template argument \removed{shall} return\added{s} a value that is convertible to \tcode{bool}.
If a template parameter is named \tcode{Clock},
the corresponding template argument \removed{shall be}\added{is} a type \tcode{C}
...
\tcode{system_error}\iref{syserr.syserr}. Such exceptions \removed{shall}\added{will} be thrown if
...
meeting its specifications. Failure to allocate storage \removed{shal bel}\added{is} reported as described
...
\removed{shall} compare\added{s} equal to one of the conditions specified in the function's error condition
element.
...
$D_m$. The delay durations \removed{may}\added{can} vary from timeout to timeout, but in all cases shorter is better.
...
since the timeout is already satisfied. \begin{note} This specification \removed{may}\added{can} result in the total
...
backwards, this specification \removed{may}\added{can} result in the total duration of the wait increasing when
measured against a steady clock. When the clock is adjusted forwards, this specification \removed{may}\added{can}
result in the total duration of the wait decreasing when measured against a steady clock.
\end{note}
\end{itemize}

An implementation \removed{shall} return\added{s} from such a timeout at any point from the time specified above to
...
\pnum
Implementation-provided clocks that are used for these functions \removed{shall satisfy}\added{meet} the
...
\pnum
An \defn{execution agent} is an entity such as a thread that \removed{may}\added{can} perform work in parallel with
other execution agents. \begin{note} Implementations or users \removed{may}\added{can} introduce other kinds of
...
\begin{note} The nature of any lock ownership and any synchronization it \removed{may}\added{could} entail are not part
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The current execution agent \removed{shall} hold\added{s} a lock on \tcode{m}.
...
execution. That representation \removed{may}\added{can} be transferred to other \tcode{thread} objects in such a way
...
The library \removed{may}\added{can} reuse the value of a \tcode{thread::id} of a terminated thread that can no longer be joined.
...
\begin{itemdescr}
\removed{\pnum\effects Constructs an object of type \tcode{id}.}
...
\begin{itemdescr}
\pnum\removed{\effects Constructs a \tcode{thread}}\added{The} object \removed{that} does not represent a thread of execution.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\constraints}\ \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} 
\removed{shall satisfy}\added{meet} the
...
\removed{shall be}\added{is} a valid expression.}
...
\added{\pnum}
\removed{\remarks}\added{\constraints} 
\removed{This constructor shall not participate in overload resolution if} \tcode{remove_cvref_t<F>}
is \added{not} the same type as \tcode{std::thread}.

\pnum
\effects\ \removed{Constructs an object of type \tcode{thread}.} The new thread of execution executes
\tcode{%
\placeholdernc{INVOKE}(\brk{}%
...
termi\-nates with an uncaught exception, \tcode{terminate} \removed{shall be}\added{is} called.
...
\begin{itemdescr}
\pnum
\removed{\effects Constructs an object of type \tcode{thread} from \tcode{x}, and sets
...
implementation \removed{shall} release\added{s} any owned resources.
...
execution. No other thread of execution \removed{shall have}\added{has} this id and this 
thread of execution \removed{shall}\added{will} always have this id. 
The object returned \removed{shall}\added{does} not compare equal to a default constructed
\tcode{thread::id}.
\end{itemdescr}
...
They \removed{shall} satisfy the requirements set out in this subclause. In this description, \tcode{m}
denotes an object of a mutex type.

\pnum
The mutex types \removed{shall satisfy}\added{meet} the \oldconcept{Lockable} requirements\iref{thread.req.lockable.req}.

\pnum
The mutex types \removed{shall be}\added{meet} \oldconcept{DefaultConstructible} and \oldconcept{Destructible}. If
initialization of an object of a mutex type fails, an exception of type
\tcode{system_error} \removed{shall be}\added{is} thrown. The mutex types \removed{shall}\added{are}
not be copyable or movable.

\pnum
The error conditions for error codes, if any, reported by member functions of the mutex types
\removed{shall be}\added{are as follows}:
\begin{itemize}
...
\pnum
The implementation \removed{shall} provide\added{s} lock and unlock operations, as described below.
...
a single mutex \removed{shall} appear to occur in a single total order. \begin{note} This
...
\pnum
The expression \tcode{m.lock()} \removed{shall be}\added{is} well-formed 
and \removed{have}\added{has} the following semantics:

\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If \tcode{m} is of type \tcode{mutex}, \tcode{timed_mutex},
...
\pnum
\sync Prior \tcode{unlock()} operations on the same object \removed{shall}
...
\pnum
The expression \tcode{m.try_lock()} \removed{shall be}\added{is} well-formed and 
\removed{have}\added{has} the following semantics:
...
immediately returns. An implementation \removed{may}\added{can} fail to obtain the lock even if it is not
...
\pnum
The expression \tcode{m.unlock()} \removed{shall be} well-formed and \removed{have}\added{has} the following semantics:

\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread \removed{shall} own\added{s} the mutex.
...
\pnum
The class \tcode{mutex} \removed{shall satisfy}\added{satisfies} all of the mutex
requirements\iref{thread.mutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.

\pnum
\begin{note} A program \removed{may}\added{can} deadlock if the thread that owns a \tcode{mutex} object calls
\tcode{lock()} on that object. If the implementation can detect the deadlock,
a \tcode{resource_deadlock_would_occur} error condition \removed{may}\added{can} be observed. \end{note}
...
\pnum
The class \tcode{recursive_mutex} \removed{shall satisfy}\added{satisfies} all of the mutex
requirements\iref{thread.mutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.

\pnum
A thread that owns a \tcode{recursive_mutex} object \removed{may}\added{can} acquire additional levels of
ownership by calling \tcode{lock()} or \tcode{try_lock()} on that object. It is
unspecified how many levels of ownership \removed{may}\added{can} be acquired by a single thread. If a thread
has already acquired the maximum level of ownership for a \tcode{recursive_mutex}
object, additional calls to \tcode{try_lock()} \removed{shall}\added{will} fail, and additional calls to
\tcode{lock()} \removed{shall}\added{will} throw an exception of type \tcode{system_error}. A thread
...
released \removed{may}\added{can} ownership be acquired by another thread.
...
\tcode{recursive_timed_mutex}, and \tcode{shared_timed_mutex}. They \removed{shall meet}\added{satisfy}
...
\pnum
The timed mutex types \removed{shall satisfy}\added{meet} the \oldconcept{TimedLockable}
requirements\iref{thread.req.lockable.timed}.

\pnum
The expression \tcode{m.try_lock_for(rel_time)} \removed{shall be}\added{is} well-formed 
and \removed{have}\added{has} the following semantics:

\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If \tcode{m} is of type \tcode{timed_mutex} or
...
\tcode{try_lock()}). The function \removed{shall} return\added{s} within the timeout specified by
...
\pnum
The expression \tcode{m.try_lock_until(abs_time)} \removed{shall be}\added{is} well-formed
and \removed{have}\added{has} the following semantics:
...
without blocking (as if by calling \tcode{try_lock()}). The function \removed{shall}
return\added{s} before the absolute timeout\iref{thread.req.timing} specified by
...
\pnum
The class \tcode{timed_mutex} \removed{shall satisfy}\added{satisfies} all of the timed mutex
requirements\iref{thread.timedmutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.
...
\pnum
The class \tcode{recursive_timed_mutex} \removed{shall satisfy}\added{satisfies} all of the timed mutex
requirements\iref{thread.timedmutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.

\pnum
A thread that owns a \tcode{recursive_timed_mutex} object \removed{may}\added{can} acquire additional
levels of ownership by calling \tcode{lock()}, \tcode{try_lock()},
\tcode{try_lock_for()}, or \tcode{try_lock_until()} on that object. It is
unspecified how many levels of ownership \removed{may}\added{can} be acquired by a single thread. If
...
\tcode{try_lock_for()}, or \tcode{try_lock_until()} \removed{shall}\added{will} fail, and additional
calls to \tcode{lock()} \removed{shall}\added{will} throw an exception of type \tcode{system_error}. A
thread \removed{shall} call\added{s} \tcode{unlock()} once for each level of ownership acquired by
...
\removed{may}\added{can} ownership of the object be acquired by another thread.
...
are \defn{shared mutex types}. Shared mutex types \removed{shall satisfy}\added{meet} the requirements of
mutex types\iref{thread.mutex.requirements.mutex}\removed{,} and \removed{additionally
shall satisfy}\added{meet} the requirements set out below. In this description,
...
execution agent \removed{shall} hold\added{s} a shared lock while another execution agent holds an
...
mutex type is unspecified, but \removed{shall be}\added{is} at least 10000. If more than the
maximum number of execution agents attempt to obtain a shared lock, the
excess execution agents \removed{shall} block until the number of shared locks are
reduced below the maximum amount by other execution agents releasing their
shared lock.

\pnum
The expression \tcode{m.lock_shared()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the
following semantics:

\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread has no ownership of the mutex.
...
\pnum
\sync Prior \tcode{unlock()} operations on the same object \removed{shall} synchronize with\iref{intro.multithread} this operation.
...
\pnum
The expression \tcode{m.unlock_shared()} \removed{shall be}\added{is} well-formed 
and \removed{have}\added{has} the following semantics:

\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread \removed{shall} hold\added{s} a shared lock on the mutex.
...
\pnum
The expression \tcode{m.try_lock_shared()} \removed{shall be}\added{is} well-formed and 
\removed{have}\added{has} the following semantics:
...
\removed{may}\added{can} fail to obtain the lock even if it is not held by any other thread.
...
\pnum
The class \tcode{shared_mutex} \removed{shall satisfy}\added{meets} all of the
shared mutex requirements\iref{thread.sharedmutex.requirements}.
It \removed{shall be}\added{is} a standard-layout class\iref{class.prop}.
...
\pnum
\tcode{shared_mutex} \removed{may}\added{can} be a synonym for \tcode{shared_timed_mutex}.
...
\defn{shared timed mutex type}. Shared timed mutex types \removed{shall satisfy}\added{meet} the requirements of
...
\removed{shall satisfy}\added{meet} the requirements set out below. In this description,
...
\pnum
The expression \tcode{m.try_lock_shared_for(rel_time)} \removed{shall be} well-formed and
\removed{have}\added{is} the following semantics:
...
\tcode{try_lock_shared()}). The function \removed{shall} return\added{s} within the timeout
...
If an exception is thrown then a shared lock \removed{shall not have}\added{has not} been acquired for
the current thread.
...
\pnum
The expression \tcode{m.try_lock_shared_until(abs_time)} \removed{shall be}\added{is} well-formed
and \removed{have}\added{is} the following semantics:
...
function \removed{shall} return\added{s} before the absolute timeout\iref{thread.req.timing}
...
\pnum
The class \tcode{shared_timed_mutex} \removed{shall satisfy}\added{meets} all of the
...
\pnum
A \term{lock} is an object that holds a reference to a lockable object and \removed{may}\added{can} unlock the
lockable object during the lock's destruction (such as when leaving block scope). An execution
agent \removed{may}\added{can} use a lock to aid in managing ownership of a lockable object in an exception safe
...
object. The supplied \tcode{Mutex} type \removed{shall satisfy}\added{meets}
the \oldconcept{BasicLockable} requirements\iref{thread.req.lockable.basic}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex,
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread owns the mutex \tcode{m}.
...
\removed{shall satisfy}\added{meets} the \oldconcept{BasicLockable} requirements\iref{thread.req.lockable.basic}.
Otherwise, each of the mutex types
\removed{shall satisfy}\added{meet} the \oldconcept{Lockable} requirements\iref{thread.req.lockable.req}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If a \tcode{MutexTypes} type is not a recursive mutex,
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread owns all the mutexes in \tcode{m}.
...
object within a scope. Ownership of the lockable object \removed{may}\added{can} be acquired at
construction or after construction, and \removed{may}\added{can} be transferred, after
...
\tcode{Mutex} type \removed{shall satisfy}\added{meet} the \oldconcept{BasicLockable}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.

\pnum
\effects \removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.lock()}.
...
requirements\iref{thread.req.lockable.req}.}
\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.

\pnum
\effects \removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock()}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread owns the mutex.
...
\begin{itemdescr}
\pnum
\added{\mandates The supplied \tcode{Mutex} type meets the
\oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.}

\added{\pnum}
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex 
the calling thread does not own the mutex. 
\removed{The supplied \tcode{Mutex} type shall satisfy the
\oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.}

\pnum
\effects \removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock_until(abs_time)}.
...
\begin{itemdescr}
\pnum
\added{\mandates The supplied \tcode{Mutex} type meets the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.}

\added{\pnum}
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.
\removed{The supplied \tcode{Mutex} type shall satisfy the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.}

\pnum
\effects \removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock_for(rel_time)}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} The supplied \tcode{Mutex} 
\removed{shall satisfy}\added{meets} the \oldconcept{Lockable}
requirements\iref{thread.req.lockable.req}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} The supplied \tcode{Mutex} type 
\removed{shall satisfy}\added{meets} the \oldconcept{TimedLockable}
requirements\iref{thread.req.lockable.timed}.
...
\pnum
\removed{\requires}\added{\mandates} The supplied \tcode{Mutex} type 
\removed{shall satisfy}\added{meets} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
lockable object within a scope. Shared ownership of the lockable object \removed{may}\added{can} be
acquired at construction or after construction, and \removed{may}\added{can} be transferred, after
...
\tcode{Mutex} type \removed{shall satisfy}\added{meets} the shared mutex
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread does not own the mutex for any ownership mode.

\pnum
\effects \removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls} \tcode{m.lock_shared()}.
...
\pnum
\effects \removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls} \tcode{m.try_lock_shared()}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The calling thread has shared ownership of the mutex.
...
\pnum
\effects \removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} Each template parameter type 
\removed{shall satisfy}\added{meets} the \oldconcept{Lockable} requirements. \begin{note} The
...
\pnum
\removed{\requires}\added{\mandates} Each template parameter type 
\removed{shall satisfy}\added{meets} the \oldconcept{Lockable} requirements,
\begin{note} The
\tcode{unique_lock} class template meets these requirements when suitably instantiated.
\end{note}
...
\begin{itemdescr}
\removed{\pnum\effects Constructs an object of type \tcode{once_flag}.}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates}
\begin{codeblock}
@\placeholdernc{INVOKE}@(std::forward<Callable>(func), std::forward<Args>(args)...)
\end{codeblock}
(see \ref{func.require}) \removed{shall be}\added{is} a valid expression.
...
is an \term{active} execution. An active execution \removed{shall} call\added{s}
...
An exceptional execution \removed{shall} propagate\added{s} the exception to the caller of
\tcode{call_once}. Among all executions of \tcode{call_once} for any given
\tcode{once_flag}: at most one \removed{shall be}\added{is} a returning execution; if there is a
returning execution, it \removed{shall be}\added{is} the last active execution; and there are
...
\pnum
The execution of \tcode{notify_one} and \tcode{notify_all} \removed{shall be}\added{are} atomic. The
execution of \tcode{wait}, \tcode{wait_for}, and \tcode{wait_until} \removed{shall be}\added{are} performed
...
\pnum
The implementation \removed{shall} behave\added{s} as if all executions of \tcode{notify_one}, \tcode{notify_all}, and each
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{lk} is locked by the calling thread and either
...
the current thread have been destroyed. This notification \removed{shall be}\added{is} as if:
\begin{codeblock}
lk.unlock();
cond.notify_all();
\end{codeblock}
...
\pnum
The class \tcode{condition_variable} \removed{shall be}\added{is} a standard-layout class\iref{class.prop}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} There \removed{shall be}\added{is} no thread blocked on \tcode{*this}.
\begin{note} That is, all
threads \removed{shall} have been notified; they \removed{may}\added{could} subsequently block on the lock specified in the
wait.
...
\removed{\pnum\effects Destroys the object.}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{lock.owns_lock()} is \tcode{true} and \tcode{lock.mutex()}
...
\removed{shall be}\added{is} called\iref{except.terminate}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{lock.owns_lock()} is \tcode{true} and \tcode{lock.mutex()} is
...
\item
If the function exits via an exception, \tcode{lock.lock()} \removed{shall be}\added{is} called prior to exiting the function.
\end{itemize}
...
\pnum
A \tcode{Lock} type \removed{shall satisfy}\added{meets} the \oldconcept{BasicLockable}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} There \removed{shall be}\added{is} no thread blocked on \tcode{*this}. \begin{note} That is, all
threads \removed{shall} have been notified; they \removed{may}\added{could} subsequently block on the lock specified in the
wait.
This relaxes the usual rules, which would have required all wait calls to happen before
destruction. Only the notification to unblock the wait needs to happen before destruction.
...
virtual function \removed{shall} return\added{s} a pointer to the string \tcode{"future"}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs an object of class \tcode{future_error}
and initializes}\added{Initializes} \tcode{ec_} with \tcode{make_error_code(e)}.
\end{itemdescr}
...
\removed{may}\added{can} block if all of the following are true: the shared state was created by a call to
...
retrieval.
Waiting for a shared state to become ready \removed{may}\added{can} invoke code to compute the result on
...
\pnum
Access to the result of the same shared state \removed{may}\added{can} conflict\iref{intro.multithread}.
...
\pnum
The implementation \removed{shall} provide\added{s} the template \tcode{promise} and two specializations,
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} \tcode{Alloc} \removed{shall satisfy}\added{meets} the \oldconcept{Allocator}
...
\begin{itemdescr}
\pnum
\effects Constructs \removed{a \tcode{promise} object and} a shared state. The second
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a new \tcode{promise} object and transfers}\added{Transfers} ownership of the shared state
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{p} is not null.
...
\pnum
The implementation \removed{shall} provide\added{s} the template \tcode{future} and two specializations,
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs an \defnx{empty}{empty \tcode{future} object}
\tcode{future}}\added{The} object \removed{that} does not refer to a
shared state.
...
which \removed{may}\added{can} share their shared state with other asynchronous return
...
\pnum
The implementation \removed{shall} provide\added{s} the template \tcode{shared_future} and two
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs an \defnx{empty}{empty \tcode{shared_future} object}
\tcode{shared_future}}\added{The} object \removed{that} does not refer to a
shared state.
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{shared_future} object that refers}\added{Refers} to the same
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} \tcode{F} and each $\tcode{T}_i$ in \tcode{Args}
\removed{shall satisfy}\added{meet} the
...
more than one of these conditions applies, the implementation \removed{may}\added{can} choose any of
...
this shared state \removed{shall} invoke\added{s} the
...
other code that uses the future should be aware that the future's destructor \removed{may}\added{can}
...
the \tcode{async} call uses the default policy, which \removed{may}\added{can} use
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{packaged_task}}\added{The} object \removed{with}\added{has} no shared state and no stored task.
\end{itemdescr}
...
of the corresponding types in \tcode{ArgTypes...}, \removed{shall be}\added{is} a valid expression. Invoking
a copy of \tcode{f} \removed{shall} behave\added{s} the same as invoking \tcode{f}.

\pnum
\removed{\remarks
This constructor shall not participate in overload resolution if}
\added{\constraints} \tcode{remove_cvref_t<F>}
is \added{not} the same type as \tcode{packaged_task<R(ArgTypes...)>}.

\pnum
\effects \removed{Constructs a new \tcode{packaged_task} object with a shared state and
initializes}\added{Initializes} the object's stored task with \tcode{std::forward<F>(f)}.
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a new \tcode{packaged_task} object and transfers}\added{Transfers} ownership of
...
case, this \removed{shall be}\added{is} done without making that state ready\iref{futures.state} immediately. Schedules
