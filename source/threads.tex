...
\begin{note}
This specification \removed{may}\added{can} result in the total
...
\begin{note}
Implementations or users \removed{may}\added{can} introduce other kinds of
...
\begin{note}
The nature of any lock ownership and any synchronization it \removed{may} entail\added{s} are not part
...
\pnum
\effects
\removed{Constructs a \tcode{thread}}\added{The} object does not represent a thread of execution.
...
\pnum
\removed{\requires}\added{\expects}\ \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} shall meet the
...
\pnum
\effects
\removed{Constructs an object of type \tcode{thread}.} The new thread of execution executes
\tcode{%
\placeholdernc{INVOKE}(\brk{}%
...
\pnum
\effects
Initializes \tcode{ssource} \removed{and
...
the implementation \removed{shall} release\added{s} any owned resources.
...
execution. No other thread of execution \removed{shall have}\added{has} this id and this  thread of execution \removed{shall}
always \removed{have}\added{has} this id. The object returned \removed{shall}\added{does} not compare equal to a default constructed
\tcode{thread::id}.
\end{itemdescr}
...
\tcode{system_error} \removed{shall be}\added{is} thrown. The mutex types \removed{shall not be}\added{are neither}
copyable \added{n}or movable.
...
a single mutex \removed{shall} appear\added{s} to occur in a single total order. 
\begin{note}
This
...
\pnum
The expression \tcode{m.lock()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the following semantics:
...
\pnum
\sync
Prior \tcode{unlock()} operations on the same object \removed{shall}
...
\pnum
The expression \tcode{m.try_lock()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the following semantics:
...
\pnum
The class \tcode{mutex} \removed{shall} meet\added{s} all of the mutex
requirements\iref{thread.mutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.

\pnum
\begin{note}
A program \removed{may}\added{can} deadlock if the thread that owns a \tcode{mutex} object calls
\tcode{lock()} on that object. If the implementation can detect the deadlock,
a \tcode{resource_deadlock_would_occur} error condition \removed{may}\added{can} be observed.
\end{note}
...
\pnum
The class \tcode{recursive_mutex} \removed{shall} meet\added{s} all of the mutex
requirements\iref{thread.mutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.
...
\tcode{recursive_timed_mutex}, and \tcode{shared_timed_mutex}. They \removed{shall}
...
\pnum
The timed mutex types \removed{shall} meet the \oldconcept{TimedLockable}
...
\pnum
The class \tcode{timed_mutex} \removed{shall} meet\added{s} all of the timed mutex
...
\pnum
The class \tcode{recursive_timed_mutex} \removed{shall} meet\added{s} all of the timed mutex
requirements\iref{thread.timedmutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.
...
are \defn{shared mutex types}. Shared mutex types \removed{shall} meet the requirements of
mutex types\iref{thread.mutex.requirements.mutex}\removed{,} and additionally
\removed{shall} meet the requirements set out below. In this description,
...
If an exception is thrown then a shared lock \removed{shall not have}\added{has not} been acquired for the current thread.
...
\pnum
\sync
Prior \tcode{unlock()} operations on the same object \removed{shall} synchronize with\iref{intro.multithread} this operation.
...
\pnum
The expression \tcode{m.unlock_shared()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the following semantics:
...
\pnum
The expression \tcode{m.try_lock_shared()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the following semantics:
...
\pnum
The class \tcode{shared_mutex} \removed{shall} meet\added{s} all of the
...
\defn{shared timed mutex type}. Shared timed mutex types \removed{shall} meet the requirements of
...
\pnum
The class \tcode{shared_timed_mutex} \removed{shall} meet\added{s} all of the
...
\pnum
\effects
\removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.lock()}.
...
\pnum
\effects
\removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock()}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread
...
\pnum
\effects
\removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock_until(abs_time)}.
...
\pnum
\effects
\removed{Constructs an object of type \tcode{unique_lock} and calls}\added{Calls} \tcode{m.try_lock_for(rel_time)}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} shall meet the \oldconcept{Lockable}
requirements\iref{thread.req.lockable.req}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type shall meet the \oldconcept{TimedLockable}
requirements\iref{thread.req.lockable.timed}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type shall meet the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\pnum
\effects
\removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls} \tcode{m.lock_shared()}.
...
\pnum
\effects
\removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls} \tcode{m.try_lock_shared()}.
...
\pnum
\effects
\removed{Constructs an object of type \tcode{shared_lock} and calls}\added{Calls}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} Each template parameter type shall meet the \oldconcept{Lockable} requirements.
\begin{note}
The
...
\added{\mandates \tcode{is_invocable_v<Callable, Args...>} is \tcode{true}.}
...
\pnum
\effects
\removed{Constructs an object of class \tcode{future_error}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{Alloc} shall meet the \oldconcept{Allocator}
...
\pnum
\effects
\removed{Constructs a \tcode{promise} object and}\added{Creates} a shared state. The second
...
\pnum
\effects
\removed{Constructs a new \tcode{promise} object and transfers}\added{Transfers} ownership of the shared state
...
\pnum
\effects
\removed{Constructs an \defnx{empty}{empty \tcode{future} object}
...
\pnum
\effects
 \removed{Constructs an \defnx{empty}{empty \tcode{shared_future} object}
...
\pnum
\effects
\removed{Constructs a \tcode{shared_future} object that refers}\added{The object refers} to the same
...
\pnum
\effects
\removed{Constructs a \tcode{packaged_task} object with}\added{The object has} no shared state and no stored task.
\end{itemdescr}
...
\added{\mandates \tcode{is_invocable_r_v<R, F\&, ArgTypes...>} is \tcode{true}.}
...
\pnum
\effects
\removed{Constructs a new \tcode{packaged_task} object with a shared state and
...
\pnum
\effects
\removed{Constructs a new \tcode{packaged_task} object and transfers}\added{Transfers} ownership of
