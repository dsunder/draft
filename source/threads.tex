...
\tcode{system_error}\iref{syserr.syserr}. Such exceptions \removed{shall be}\added{are} thrown if
...
\begin{itemdescr}
\pnum\effects \removed{Constructs a \tcode{thread}}\added{The} object does not represent a thread of execution.
...
\pnum
\removed{\requires}\added{\expects}\ \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} 
...
thread of execution \removed{shall} always \removed{have}\added{has} this id. 
...
They \removed{shall satisfy}\added{meet} the requirements set out in this subclause. In this description, \tcode{m}
...
not be copyable \added{n}or movable.
...
a single mutex \removed{shall} appear\added{s} to occur in a single total order. \begin{note} This
...
\pnum
The expression \tcode{m.unlock()} \removed{shall be}\added{is} well-formed and \removed{have}\added{has} the following semantics:
...
\pnum
The class \tcode{mutex} \removed{shall satisfy}\added{meets} all of the mutex
...
\pnum
The class \tcode{recursive_mutex} \removed{shall satisfy}\added{meets} all of the mutex
requirements\iref{thread.mutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.
...
object, additional calls to \tcode{try_lock()} \removed{shall} fail, and additional calls to
\tcode{lock()} \removed{shall} throw an exception of type \tcode{system_error}. A thread
...
\tcode{recursive_timed_mutex}, and \tcode{shared_timed_mutex}. They \removed{shall} meet
...
\pnum
The timed mutex types \removed{shall satisfy}\added{meets} the \oldconcept{TimedLockable}
...
\pnum
The class \tcode{timed_mutex} \removed{shall satisfy}\added{meets} all of the timed mutex
...
\pnum
The class \tcode{recursive_timed_mutex} \removed{shall satisfy}\added{meets} all of the timed mutex
requirements\iref{thread.timedmutex.requirements}. It \removed{shall be}\added{is} a standard-layout
class\iref{class.prop}.
...
\tcode{try_lock_for()}, or \tcode{try_lock_until()} \removed{shall} fail, and additional
calls to \tcode{lock()} \removed{shall} throw an exception of type \tcode{system_error}. A
...
mutex types\iref{thread.mutex.requirements.mutex}\removed{,} and additionally
...
\pnum
The class \tcode{shared_mutex} \removed{shall satisfy}\added{meet} all of the
...
\pnum
The expression \tcode{m.try_lock_shared_for(rel_time)} \removed{shall be}\added{is} well-formed and
\removed{have}\added{has} the following semantics:
...
\tcode{Mutex} type \removed{shall satisfy}\added{meets} the \oldconcept{BasicLockable}
...
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{meets} the \oldconcept{Lockable}
...
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{meets} the
...
\pnum
\removed{\requires}\added{\expects} If \tcode{mutex_type} is not a recursive mutex the calling thread does not own the mutex.
The supplied \tcode{Mutex} type \removed{shall satisfy}\added{meets} the \oldconcept{TimedLockable} requirements\iref{thread.req.lockable.timed}.
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} 
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} The supplied \tcode{Mutex} type 
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} Each template parameter type 
...
\pnum
The execution\added{s} of \tcode{notify_one} and \tcode{notify_all} 
\removed{shall be}\added{are} atomic.
The execution\added{s} of \tcode{wait}, \tcode{wait_for}, and \tcode{wait_until} \removed{shall be}\added{are} performed
...
the current thread have been destroyed. This notification \removed{shall be as if}\added{is equivalent to}:
\begin{codeblock}
lk.unlock();
cond.notify_all();
\end{codeblock}
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\expects} \tcode{Alloc} \removed{shall satisfy}\added{meets} the \oldconcept{Allocator}
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{promise} object and}\added{Creates} a shared state. The second
...
\tcode{future} object that does}\added{The object does} not refer to a
shared state.
...
\tcode{shared_future} object that does}\added{The object does} not refer to a
shared state.
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{shared_future} object that refers}\added{The object refers} to the same
...
\begin{itemdescr}
\pnum
\removed{\requires}\added{\mandates} \removed{\tcode{F} and each $\tcode{T}_i$ in \tcode{Args}
shall satisfy the \oldconcept{MoveConstructible} requirements, and}
\begin{codeblock}
...
\added{\pnum
\expects \tcode{F} and each $\tcode{T}_i$ in \tcode{Args} meet the \oldconcept{MoveConstructible} requirements.}
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{packaged_task} object with}\added{The object has} no shared state and no stored task.
\end{itemdescr}
...
\added{\pnum
\constraints \\ \tcode{remove_cvref_t<F>}
...
of the corresponding types in \tcode{ArgTypes...}, \removed{shall be}\added{is} a valid expression.\removed{ Invoking
a copy of \tcode{f} shall behave the same as invoking \tcode{f}.}

\added{\pnum
\expects Invoking a copy of \tcode{f} behaves the same as invoking \tcode{f}.}
