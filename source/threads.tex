...
\begin{itemdescr}
\pnum\effects \removed{Constructs a \tcode{thread} object does}\added{Does} not represent a thread of execution.
...
the current thread have been destroyed. This notification \removed{shall be as if}\added{is equilivant to}:
\begin{codeblock}
lk.unlock();
cond.notify_all();
\end{codeblock}
...
\tcode{future} object that does}\added{Does} not refer to a
shared state.
...
\tcode{shared_future} object that does}\added{Does} not refer to a
shared state.
...
\begin{itemdescr}
\pnum
\effects \removed{Constructs a \tcode{packaged_task} object with}\added{Has} no shared state and no stored task.
\end{itemdescr}
