...
\begin{itemdescr}
\returns Returns \tcode{false} if the type \tcode{Lock} is equivalent to \tcode{atomic_ref_assume_lock_free_t}
...
\throws Nothing 
\begin{addedblock}
if type \tcode{never_requires_user_lock} is \tcode{true}, or if \tcode{Lock} is
either equivalent to \\ \tcode{atomic_ref_assume_lock_free_t} or 
\tcode{atomic_ref_assume_no_user_lock_t}.
Otherwise, throws \\ \tcode{\textcolor{blue}{????}_error} if 
...
\tcode{Lock} is equivalent to \tcode{atomic_ref_assume_lock_free_t}, or
\tcode{Lock} is equivalent to \tcode{atomic_ref_assume_no_user_lock_t}.
\\ Furthermore, the object referenced \tcode{obj} is be aligned to \tcode{required_lock_free_alignment} and
this constructor is used for all \tcode{atomic_ref} objects which concurrently reference the 
object referenced by \tcode{obj}.
...
\tcode{Lock} is equivalent to \tcode{atomic_ref_assume_lock_free_t}, or
\tcode{Lock} is equivalent to \tcode{atomic_ref_assume_no_user_lock_t}.
\\ Furthermore, for all \tcode{atomic_ref} objects which concurrently reference the object referenced
...
\tcode{Lock} is equivalent to \tcode{atomic_ref_assume_no_user_lock_t}.
\\ Furthermore, for all \tcode{atomic_ref} objects which concurrently reference the object referenced
by \tcode{obj} either the following conditions are true:
...
\begin{codeblock}
  ...
  ulock = requires_user_lock(obj) ? 
    &ranges::begin(lks)[std::hash(&obj)%ranges::size(lks)] : nullptr;
\end{codeblock}
...
\begin{codeblock}
  ...
  ulock = is_lock_free(obj) ? 
    nullptr : &ranges::begin(lks)[std::hash(&obj)%ranges::size(lks)];
\end{codeblock}
...
namespace std {
  ...
  public:
    ...
    explicit atomic_ref(T&) noexcept@\added{(\textit{see above})}@;
