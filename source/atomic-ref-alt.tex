...
namespace std {
  ...
  struct atomic_ref_assume_no_user_lock_t {
    explicit constexpr atomic_ref_assume_no_user_lock_t() noexcept = default;
  };
...
\begin{itemize}
\item be equivalent to \tcode{atomic_ref_assume_lock_free_t},
\item be equivalent to \tcode{atomic_ref_assume_no_user_lock_t}, or
\item the type \tcode{lock_type} meets the \oldconcept{BasicLockable} requirements.  
\end{itemize}
Dianostics are required if \tcode{lock_type} is equivalent to \tcode{atomic_ref_assume_lock_free_t} 
...
Diagnostics are required if \tcode{lock_type} is equivalent to \\
\tcode{atomic_ref_assume_no_user_lock_t} 
and the implementation does not provide atomic
...
the destructor, and \\ \tcode{is_lock_free()} the following is \tcode{true}:
\begin{itemize}
\item If \tcode{ulock} points to a valid \tcode{lock_type} object which meets the 
  ...
  \begin{note}If \tcode{lock_type} does not meet the \oldconcept{BasicLockable} requirements then 
...
Returns \tcode{true} if \tcode{lock_type} is equivalent to \\
\tcode{atomic_ref_assume_lock_free_t}.
