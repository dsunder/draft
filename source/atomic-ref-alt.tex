...
namespace std {
  ...
  public:
    ...
    @\added{static constexpr bool never_requires_user_lock = \impdefx{whether a given \tcode{atomic_ref} type's operations never require a user provided lock};}@
...
\added{on objects aligned to \tcode{required_alignment}},
...
\begin{itemdescr}
\pnum
The static data member \tcode{never_requires_user_lock} is \tcode{true}
if the \tcode{atomic_ref} type's operations never require the user to provide a
valid pointer to a \tcode{Lock} object. 
\end{itemdescr}
\end{addedblock}
...
\begin{itemdescr}
\returns Returns \tcode{true} if \tcode{atomic_ref} requires the user to
provide a valid pointer to a \tcode{Lock} object when constructing an 
\tcode{atomic_ref} from \tcode{obj}. 
\begin{note}
Implementations could provide ways for \tcode{atomic_ref} to use 
implementation defined locking mechanisms when \tcode{is_lock_free(obj)} is \tcode{false}.
\end{note}
\end{itemdescr}
\end{addedblock}
...
\begin{itemdescr}
\pnum
\removed{\requires The referenced object shall be aligned to \tcode{required_alignment}} 
\begin{addedblock}
\expects The type \tcode{Lock} meets the \oldconcept{BasicLockable} requirements
and \tcode{lk} points to the  the same address for all \tcode{atomic_ref} which 
reference the object referenced by \tcode{obj}.
If \tcode{is_lock_free(obj)} is \tcode{false} and \tcode{requires_user_lock(obj)} is \tcode{false}
then \tcode{lk} points to either a valid \tcode{Lock} object or \tcode{nullptr}.
If \tcode{requires_user_lock} is \tcode{true} then \tcode{lk} points to valid \tcode{Lock} object.
\end{addedblock}
...
\begin{addedblock}
\effects If \tcode{lock} is equivalent to \tcode{nullptr}
...
\begin{addedblock}
\pnum
\effects If \tcode{lock} is equivalent to \tcode{nullptr} then the
...
with \tcode{desired}.}
\begin{addedblock}
\effects \added{If \tcode{lock} is equivalent to \tcode{nullptr}}
then the operation is equivalent to atomically performing the following:
\begin{codeblock}
  T result;
  memcpy(&result, ptr, sizeof(T));
...
\begin{addedblock}
If \tcode{lock} is equivalent to \tcode{nullptr} then
